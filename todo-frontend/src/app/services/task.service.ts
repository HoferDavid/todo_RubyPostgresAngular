...
export class TaskService {
  ...
  async loadTasks() {
    try {
      ...
      this.error.set(`Failed to load tasks: ${err instanceof Error ? err.message : 'Unknown error'}`);
    }
  }
  ...
  async addTask(task: Task) {
    try {
      ...
      this.error.set(`Failed to add task: ${err instanceof Error ? err.message : 'Unknown error'}`);
    }
  }
}